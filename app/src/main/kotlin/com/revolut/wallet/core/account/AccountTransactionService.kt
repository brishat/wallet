...
class AccountTransactionService {
    ...
    suspend fun getTransactions(accountId: UUID): List<AccountTransaction> = transaction {
        AccountTransactionTable.select {
            AccountTransactionTable.accountId eq accountId
        }.toList().map { it.toAccountTransaction() }
    }

    suspend fun getTransaction(accountId: UUID, transactionId: UUID): AccountTransaction = transaction {
        AccountTransactionTable.select {
