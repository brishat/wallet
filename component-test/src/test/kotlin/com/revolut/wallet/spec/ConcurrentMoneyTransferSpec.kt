...
class ConcurrentMoneyTransferSpec : DescribeSpec({
    ...
    describe("Concurrent money transfer") {
        ...
        val account3 = createAccount(100.0)
        ...
        context("Transfer a1 -> a2, a1 -> a3") {

            it("should create transaction").config(invocations = 50, parallelism = 5) {
                val transaction1Id = transfer(account1, account2, 1)
                val transaction2Id = transfer(account1, account3, 1)
                logger.info { transaction1Id }
                logger.info { transaction2Id }
                listOfNotNull(transaction1Id, transaction2Id).size shouldBeLessThanOrEqual 2
            }
        ...
        context("Transfer a1 -> a2, a2 -> a3, a3 -> a1") {

            it("should create transaction").config(invocations = 50, parallelism = 5) {
                val transaction1Id = transfer(account1, account2, 1)
                val transaction2Id = transfer(account2, account3, 1)
                val transaction3Id = transfer(account3, account1, 3)
                logger.info { transaction1Id }
                logger.info { transaction2Id }
                logger.info { transaction3Id }
                listOfNotNull(transaction1Id, transaction2Id, transaction3Id).size shouldBeLessThanOrEqual 3
            }
